...
如果你想开始做 A 股量化，但还不知道该从哪里起步，`quant-qmt` 会带你一步一步把环境、数据、网关和交易链路跑起来：先接入 `MiniQMT` 和常用的 QMT 数据接口，再跑通查询、下单、撤单和示例策略，让你真正从 0 到 1 开始可落地的 A 股量化交易。
...
- 很多券商开通 `MiniQMT / QMT` 的常见门槛是 `10 万左右起`，也有更高要求，实际以券商和客户经理口径为准
...
> `-QmtPath` 应填写你的 MiniQMT 实际安装目录里的 `userdata_mini` 路径，例如 `C:\broker\MiniQMT\userdata_mini`。
